      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${project.name} — ${project.role ?? "independent experiment"}`,
        },
      ],
    },
    twitter: {
...
function EvidenceBadge({ isPublic }: { isPublic: boolean }) {
  return (
    <span
      className={`case-evidence-badge ${isPublic ? "public" : "documented"}`}
    >
      <i />
      {isPublic ? "PUBLIC / INSPECTABLE" : "DOCUMENTED / FIRST-PARTY"}
    </span>
  );
}
...
> = {
  bytetoken: [
    {
      label: "BENCHMARK",
      title: "benchmark_realworld.py",
      href: "https://github.com/chandanpandeys/bytetoken/blob/main/benchmarks/benchmark_realworld.py",
    },
    {
      label: "REPOSITORY",
      title: "ByteToken source + reproduction instructions",
  ...
  benchwolf: [
    {
      label: "PREFLIGHT",
      title: "Hardware fit and throughput estimation",
      href: "https://github.com/chandanpandeys/benchwolf/blob/main/src/benchwolf/preflight.py",
    },
    {
      label: "PACKAGE",
      title: "CLI entry point and package metadata",
      href: "https://github.com/chandanpandeys/benchwolf/blob/main/pyproject.toml",
    },
  ],
  dekhosuno: [
    {
      label: "REPOSITORY",
      title: "DekhoSuno public source and feature documentation",
      href: "https://github.com/chandanpandeys/DekhoSuno",
    },
    {
      label: "ARCHITECTURE",
      title: "Public architecture artifact",
  ...
  oneclickallresultsbot: [
    {
      label: "REPOSITORY",
      title: "OneClickAllResultsBot public source",
...
export default async function ProjectPage({
  ...
  const sources =
    sourceLinks[project.slug] ??
    (project.github
      ? [
          {
            label: "SOURCE",
            title: project.name + " public repository",
            href: project.github,
          },
        ]
      : []);
  ...
  const projectJsonLd = {
    ...
    ...(project.role === "Research contributor"
      ? { contributor: { "@id": `${site.canonicalUrl}/#person` } }
      : { author: { "@id": `${site.canonicalUrl}/#person` } }),
    isPartOf: { "@id": `${site.canonicalUrl}/#profile-page` },
    about: project.category,
    keywords: project.stack.join(", "),
    ...(project.github
      ? { codeRepository: project.github, sameAs: project.github }
      : {}),
  };
    ...
    <main id="main" className="inner-page case-study">
      ...
      <section className="site-shell case-hero case-hero-v2">
        ...
        <p className="source-note">
          {project.role ?? "Independent experiment"} ·{" "}
          ...
          <div>
            ...
            <p>
              {project.status === "public"
                ? "Public code + first-party documentation"
                : "First-party project documentation"}
            </p>
          </div>
          <div>
            <span>POSITION IN PORTFOLIO</span>
            <p>
              {project.tier === "flagship"
                ? "Flagship technical story"
        ...
        <article className="case-body case-body-v2">
          ...
          {project.architecture && (
            <section id="architecture" className="architecture-section">
              <div className="architecture-heading">
                <div>
                  <p className="eyebrow">03 / SYSTEM VIEW</p>
                  <h2>How the pieces connect</h2>
                </div>
                <p>
                  Architecture is shown as a reasoning aid, not decorative
          ...
          {project.metrics && (
            <section className="metric-section">
              <div className="architecture-heading">
                <div>
                  <p className="eyebrow">04 / MEASUREMENTS</p>
                  <h2>What is actually measured</h2>
                </div>
                <p>
                  Numbers retain their baseline and qualification so compressed,
                  inferred, and illustrative results do not blur together.
                </p>
              </div>
