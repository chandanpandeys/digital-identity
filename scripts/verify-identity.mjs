...
if (!profile.includes("VERCEL_PROJECT_PRODUCTION_URL"))
  throw new Error(
    "Canonical URL must derive from Vercel's stable production domain.",
  );
if (!profile.includes("NEXT_PUBLIC_SITE_URL"))
  throw new Error("Optional custom-domain override is missing.");
if (!profile.includes("http://localhost:3000"))
  throw new Error("Local canonical fallback is missing.");
...
for (const marker of [
  ...
  if (!layout.includes(marker))
    throw new Error(`Root identity metadata is missing marker: ${marker}`);
}
...
for (const marker of [
  ...
  if (!manifest.includes(marker))
    throw new Error(`Web manifest is missing marker: ${marker}`);
}

const notFound = await text("app/not-found.tsx");
if (!notFound.includes("CONTEXT MISS") || !notFound.includes("/ask"))
  throw new Error(
    "Branded not-found route lost its identity/navigation contract.",
  );
...
for (const marker of [
  ...
  if (!robots.includes(marker))
    throw new Error(`robots.txt policy is missing crawler: ${marker}`);
}
if (!robots.includes('{ userAgent: "GPTBot", disallow: "/" }'))
  throw new Error("GPTBot training opt-out is missing.");
if (!robots.includes('{ userAgent: "ClaudeBot", disallow: "/" }'))
  throw new Error("ClaudeBot training opt-out is missing.");
...
for (const route of [
  ...
  if (!sitemap.includes(`\"${route}\"`))
    throw new Error(`Sitemap is missing required route: ${route}`);
}
...
for (const marker of [
  ...
  if (!machineProfile.includes(marker))
    throw new Error(`profile.json contract is missing marker: ${marker}`);
}
...
for (const marker of [
  ...
  if (!llms.includes(marker))
    throw new Error(`llms.txt contract is missing section: ${marker}`);
}

for (const machineRoute of [
  "app/profile.json/route.ts",
  "app/evidence.json/route.ts",
  "app/llms.txt/route.ts",
]) {
  ...
  if (!contents.includes('"x-robots-tag": "noindex, noarchive"')) {
    throw new Error(
      `Machine endpoint must remain crawlable but non-indexable: ${machineRoute}`,
    );
  }
}

const resumeRoute = await text("app/resume/pdf/[variant]/route.ts");
if (!resumeRoute.includes("x-robots-tag"))
  throw new Error(
    "Generated resume PDFs must keep their noindex response header.",
  );

const projectPage = await text("app/work/[slug]/page.tsx");
if (!projectPage.includes("SoftwareSourceCode"))
  throw new Error("Project-level SoftwareSourceCode JSON-LD is missing.");

console.log(
  `Identity verification passed: ${requiredFiles.length} required surfaces, ${scanFiles.length} files scanned.`,
);
